...
makeTri;
makeTrikeff::usage = "Calculates keff for all triangles generated by makeTri";


stochp;
nonstoch;
PlinSubs;
PlinsubsT::usage = "loads pks and Tks for a given survey";
Prior;
Prior4sky;
...
SubFisher::usage = "Get sub-fisher corresponding to specific parameters";
SubFisher4sky::usage = "Get sub-fisher corresponding to specific parameters for 4 skies";
...
jmatPath = "BLOOP_JMAT_PATH"/.paths
...
triangles = Select[Tuples[ktab, {3}]// DeleteDuplicatesBy[Sort], 
    (#[[1]] + #[[2]] >= #[[3]]) &];
 Sort[triangles]
];

(*Get keff from triangle and bin size*)
MedianN[{q1_?NumericQ,q2_?NumericQ,q3_?NumericQ}] := Median[{q1,q2,q3}];
...
k2eff =1/VT NIntegrate[q1 q2 q3 MedianN[{q1,q2,q3}] Boole[q1<=q2+q3]Boole[q2<=q1+q3]Boole[q3<=q1+q2],
		{q1,k1-\[CapitalDelta]k/2,k1+\[CapitalDelta]k/2},{q2,k2-\[CapitalDelta]k/2,k2+\[CapitalDelta]k/2},{q3,k3-\[CapitalDelta]k/2,k3+\[CapitalDelta]k/2}
		];
...
Prior =(BBN\[Nu]cosmoprior+EFTprior)/.priorrep;
...
Prior4sky = buildCorrPrior[Prior, numCosmoFnlParams]/.priorrep;
